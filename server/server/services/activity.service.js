...
const insertActivity = async (req) => {
  ...
  if (activityConfig && activityConfig[req.activityKey]) {
    ...
    if (req.body && (req.body.email || req.body.emailid) && activity.contextType && (["LOGIN", "LOGINSUCCESS"].includes(activity.contextType))) {
    ...
    if (['employee'  ,'user'].includes(sessionUtil.getTokenInfo(req, "loginType"))) {
