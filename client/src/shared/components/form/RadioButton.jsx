...
class RadioButtonField extends PureComponent {
  ...
  onChange = () => {
    const { onChange, radioValue, name } = this.props;
    if (name == 'cartSublocation' || name == 'clinic') {
      this.props.getCustomLocationList(radioValue);
    }
    onChange(radioValue, this.props.name);
  };
    ...
    return (
      ...
      >
        <input
          ...
          style={{height:'10px'}}
        />
        <span className="theme-white radio-btn__radio-custom" />
        {className === 'button'
          ? (
